...
uint32_t GetLdbaseVidx(const unsigned char* vrtypes, uint32_t cur_vidx) {
  ...
  if (cur_vidx_orig_remainder) {
    VecW cur_vvec = vrtypes_valias[vidx_vec_idx];
    // non-ld: ((bit 2) OR (NOT bit 1))
    VecW cur_vvec_bit2 = vecw_slli(cur_vvec, 5);
    VecW cur_vvec_bit1 = vecw_slli(cur_vvec, 6);
    mmui = vecw_movemask(cur_vvec_bit2 | (~cur_vvec_bit1));
